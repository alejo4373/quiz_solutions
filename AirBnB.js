...
//Reading rooms.json file and storing it into var rooms
var rooms;
fs.readFile('rooms.json', (err, file) => {
    if (err) throw err;
...
//Save function saves to file and exits the program
function save(){    
    var data = JSON.stringify(rooms, null, 2);
...
function deleteRoom(index) {
    ...
    rooms = rooms.filter(function(elem) { return elem !== undefined})
   }
rl.on('line', function(input) {
    ...
    } else if(inputArr[0] === 'save') { save() }
      else if(inputArr[0] === 'quit') { process.exit() }
      else if(inputArr[0] === 'delete') { deleteRoom(inputArr[1] - 1) }
    else {
